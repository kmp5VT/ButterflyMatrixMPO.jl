struct BFMatrixMPO
        ...
        function BFMatrixMPO(factors, levels, iMPOinds, jMPOinds, hyperindsi, hyperindsj, ranks)
            ...
            sliced_index_map = Vector{Int}()
            for i in 1:length(factors)
                indset = [iMPOinds..., jMPOinds[end:-1:1]...]
                hyperindsi = commoninds(indset, keys(bitsI))
                hyperindsj = commoninds(indset, keys(bitsJ))
                indsposi = map(x->findfirst(indset, x), hyperindsi)
                indsposj = map(x->findfirst(indset, x), hyperindsj)
